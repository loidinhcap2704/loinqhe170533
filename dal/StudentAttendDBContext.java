...
public class StudentAttendDBContext extends DBContext<StudentAttend> {
    ...
    public ArrayList<StudentAttend> chooseCourseToSeeAttendReport(String username) {
        ...
        String sql = "SELECT TOP (1000)\n"
                ...
                + "	  , cname\n"
                + "	  , shortname\n"
                + "  FROM [Assignment].[dbo].[StudentAttend] sa inner join Class c\n"
                ...
                + "  on sa.[sid] = s.[sid]\n"
                + "  where username = ?\n"
                + "  group by co.cid, gname, cname, shortname";
        try {
            stm = connection.prepareStatement(sql);
            stm.setString(1, username);
            rs = stm.executeQuery();
